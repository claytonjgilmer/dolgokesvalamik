...
static void LoadLocals (LoadState* S, Proto* f)
{
 ...
 for (i=0; i<n; i++)
 {
  f->locvars[i].varname=LoadString2(S);
...
static void LoadUpvalues (LoadState* S, Proto* f)
{
 ...
 f->sizeupvalues=n;
 for (i=0; i<n; i++) f->upvalues[i]=LoadString2(S);
}
...
static Proto* LoadFunction (LoadState* S, TString* p)
{
 ...
 f->source=LoadString2(S); if (f->source==NULL) f->source=p;
